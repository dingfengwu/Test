...
namespace Kehu1688.Framework.Permission.Service
{
    public class OAuthAuthorizationServerHandler : AuthenticationHandler<OAuthAuthorizationServerOptions>
    {
        ...
        protected override async Task FinishResponseAsync()
        {
            ...
            var identities = new ClaimsPrincipal();
            ...
            if (_authorizeEndpointRequest.IsAuthorizationCodeGrantType)
            {
                ...
                if (!string.IsNullOrEmpty(_authorizeEndpointRequest.RedirectUri))
                {
                    ...
                    properties.Items[Constants.Extra.RedirectUri] = _authorizeEndpointRequest.RedirectUri;
                }

                var context = new AuthenticationTokenCreateContext(
                    Context,
                    Options.AuthorizationCodeFormat,
                    new AuthenticationTicket(Context.User, properties, Options.AuthenticationType));
            ...
            BaseValidatingContext<OAuthAuthorizationServerOptions> validatingContext)
        {
            ...
            return Response.WriteAsync(body.ToString());
        }

        private async Task<bool> InvokeAuthorizeEndpointAsync()
        {
            var authorizeRequest = new AuthorizeEndpointRequest(Context.Request.Query);
        ...
        private async Task<bool> SendErrorPageAsync(string error, string errorDescription, string errorUri)
        {
            ...
            StringBuilder body = new StringBuilder();
            body.AppendFormat("error: {0}\r\n", error);
            if (!string.IsNullOrEmpty(errorDescription))
            {
                body.AppendFormat("error_description: {0}\r\n", errorDescription);
            }
            if (!string.IsNullOrEmpty(errorUri))
            {
                body.AppendFormat("error_uri: {0}\r\n", errorUri);
            }
